...
class TestCoderunnerQuestionStreaming:
    def test_yml_parsing_non_strict(self, capsys: pytest.CaptureFixture[str]) -> None:
        ...
        sys.argv = ["make-questions", "-i", "examples/coderunner-stream.yaml", "-s"]
    ...
    def test_e2e_cli_make_question_stream(self, tmp_path: Path) -> None:
        ...
        with open(test_resources_dir / "coderunner-stream.xml", "r", encoding="utf-8") as f:
        ...
        sys.argv = [
            "make-questions",
            "-i",
            "examples/coderunner-stream.yaml",
            "-o",
