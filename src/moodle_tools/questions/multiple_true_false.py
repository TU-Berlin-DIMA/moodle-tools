...
class MultipleTrueFalseQuestion(Question):
    ...
    def extract_properties_from_xml(element: Element) -> dict[str, str | Any | None]:
        ...
        question_props.update({"scoringmethod": element.find("scoringmethod").find("text").text,
                               "shuffle_answers": element.find("shuffleanswers") == "True",
                               "answernumbering": element.find("answernumbering").text})

        choices = [c.find("responsetext").find("text").text for c in
                   sorted(element.findall("column"), key=lambda c: c.get("number"))]
        question_props.update({"choices": choices})

        answers = [{"answer": parse_html(a.find("optiontext").find("text").text),
        ...
        for w in element.findall("weight"):
            if float(w.find("value").text) == 1.0:
                answers[int(w.get("rownumber")) - 1]["choice"] = choices[int(w.get("columnnumber")) - 1]
